...
public class ListVideoCategoryActivity extends Activity implements View.OnClickListener, AdapterView.OnItemClickListener {
    ...
    List<SoundCategoryDTO> soundCategoryDTOList;
    ...
    TextWatcher textWatcher = new TextWatcher() {
        ...
        public void afterTextChanged(final Editable s) {
            ...
            timer.schedule(new TimerTask() {
                @Override
                public void run() {
                    final List<SoundCategoryDTO> categoryDTOs;
                    categoryDTOs = soundsAdapter.getListContents();
                    if (StringUtils.isNotBlank(s.toString())) {
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                ivClearText.setVisibility(View.VISIBLE);
                                List<SoundCategoryDTO> soundCategoryMatchingWithText = getSoundMatchingWithText(categoryDTOs, s.toString().toLowerCase().trim());
                        ...
                        runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                soundsAdapter.setListContents(soundCategoryDTOList);
                                soundsAdapter.notifyDataSetChanged();
                            }
                        });
                    }
    ...
    public void onEventMainThread(GetListSoundResponseEvent event) {
        soundCategoryDTOList = event.getListSoundDTOs();
        soundsAdapter = new SoundsAdapter(this, soundCategoryDTOList);
